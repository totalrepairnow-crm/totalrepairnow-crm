...
[ -f "$FILE" ] && cp -a "$FILE" "$FILE.bak.$(date +%F_%H%M%S)"

echo "[2/3] Escribiendo ClientDetail.jsx (servicios con qty, unit_price, descripción, total y eliminar)"
cat > "$FILE" <<'JSX'
...
echo "[OK] Detalle de servicios actualizado. Hard refresh (Ctrl+F5 / Cmd+Shift+R)."
