...
echo "[1/4] Instalar plugin ESLint requerido (por si el config lo usa en otros archivos)"
cd "$FRONT"
# Forzamos presencia del plugin para evitar "rule not found"
npm i -D eslint-plugin-react-hooks@^4 >/dev/null 2>&1 || true

echo "[2/4] Respaldo del ClientDetail.jsx"
mkdir -p "$FRONT/src/pages"
cp -a "$FILE" "$FILE.bak.$(date +%s)" 2>/dev/null || true

echo "[3/4] Reescribir ClientDetail.jsx SIN comentario eslint y con useEffect correcto"
cat > "$FILE" <<'EOF'
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        await loadClient();
        await Promise.all([loadServices(), loadUploads()]);
      } catch (e) {
        if (active) {
...
echo "[4/4] Build y publicación"
cd "$FRONT"
npm run build
sudo rsync -av --delete build/ /var/www/crm/
sudo systemctl reload nginx
echo "OK. Arreglado ESLint y ClientDetail. Hard refresh (Ctrl+F5/Cmd+Shift+R)."
